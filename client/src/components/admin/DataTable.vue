<template>
   ...
   >
         ...
         <v-col class="prop v-col-3 flex-center">
            ...
            >
               <span
                  class="title"
                  :key="activeProp"
                  >{{ activeProp ? activeProp : tableHeaders[0] }}</span
               >
            </Transition>
         </v-col>

         <div class="data v-col-9">
            <p
               class="title"
               style="
      ...
      <v-row class="sort-bar border-dashed border-0 border-b">
         <v-col
            cols="6"
            sm="auto"
            class="d-flex align-center"
         >
            <p class="text-body">Sắp xếp theo</p>
         </v-col>
         <v-col>
            <v-select
         ...
         <v-col
            ...
            class="d-flex align-center"
         >
            <p class="text-body">Số lượng hiển thị</p>
         </v-col>
         <v-col>
            <v-select
   ...
   export default {
      ...
      methods: {
         renderItems() {
            this.totalPages = Math.ceil(
               this.localItems.length / this.documentsPerPageSelected
            );
            ...
            this.itemsRendering = this.localItems.slice(
      ...
      watch: {
         sortCondition: {
            handler: function (newValue) {
               if (newValue.sortTypeSelected === 1) {
                  sortAscending(this.localItems, newValue.sortBySelected);
               } else sortDescending(this.localItems, newValue.sortBySelected);
               this.renderItems();
            },
            deep: true,
         },
         items() {
            ...
            if (this.sortCondition.sortTypeSelected === 1) {
               sortAscending(
                  this.localItems,
                  this.sortCondition.sortBySelected
               );
            } else {
               sortDescending(
                  this.localItems,
      ...
      created() {
         ...
         sortAscending(this.localItems, this.sortCondition.sortBySelected);
         this.renderItems();
      },
   };
</script>
